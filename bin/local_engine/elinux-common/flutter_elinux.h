...
typedef struct {
  ...
  struct _XDisplay* x_display;
  uintptr_t x_window;
} FlutterElinuxEglContextInfo;

typedef void (*FlutterElinuxOnGlContextReady)(
    void* user_data,
    const FlutterElinuxEglContextInfo* info);

typedef void (*FlutterElinuxRenderTask)(void* user_data);
...
typedef struct {
  ...
  // Use the IVI shell instead of xdg-shell (Wayland only).
  bool use_ivi_shell;

  // The IVI surface id to register when using IVI shell.
  uint32_t ivi_surface_id;

  // Target IVI layer id to render the surface into.
  uint32_t ivi_layer_id;

  // Desired top-left position of the layer on screen in physical pixels.
  int32_t ivi_surface_x;
  int32_t ivi_surface_y;
...
// Registers a callback that fires when the engine's EGL context becomes ready
// and whenever it is recreated.
FLUTTER_EXPORT void FlutterElinuxRegisterGlContextReadyCallback(
    FlutterDesktopPluginRegistrarRef registrar,
    FlutterElinuxOnGlContextReady callback,
    void* user_data);

// Populates |out_info| with the current EGL context data if available.
FLUTTER_EXPORT bool FlutterElinuxGetEglContextInfo(
    FlutterDesktopPluginRegistrarRef registrar,
    FlutterElinuxEglContextInfo* out_info);

// Posts a task to run on the engine's render thread while the EGL context is
// current.
FLUTTER_EXPORT bool FlutterElinuxPostRenderThreadTask(
    FlutterDesktopPluginRegistrarRef registrar,
    FlutterElinuxRenderTask task,
    void* user_data);
